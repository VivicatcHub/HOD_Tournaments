...
export default function RankingTable({ data, highlightIndex, ascending, }) {
    ...
    return (<div className="max-w-2xl mx-auto relative">
      <div ref={containerRef} className="relative h-[500px] overflow-y-auto overflow-x-hidden border border-gray-300 rounded-md">
        <table className="w-full table-auto border-collapse">
          ...
          <tbody>
            ...
            const enchanted = (!ascending && score > averageScore) ||
                (ascending && score < averageScore);
            const dancingClass = index < 3 ? "dancing-row" : "";
            return (<tr key={index} ref={highlightIndex === index ? rowRef : null} className={`relative text-center ${highlightIndex === index ? "bg-yellow-200 font-bold" : ""} ${dancingClass}`} style={dancingClass
      ...
      {highlightRow && isOutOfView && highlightIndex !== null && (<HighlightRow highlightIndex={highlightIndex} highlightRow={highlightRow}/>)}
    </div>);
}
