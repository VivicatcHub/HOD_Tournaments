...
export default function App() {
    ...
    const handleSearch = (query) => {
        setSearch(query);
        const index = ranking.findIndex((row) => row.Nom.toLowerCase() === query.toLowerCase());
    ...
    return (<div className="min-h-screen p-4 bg-gray-50 text-gray-800 z-5">
      <div className="sticky top-0 bg-gray-50 z-10 p-4 shadow-md rounded-xl">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-4 z-15">
          ...
          <select value={selectedSheet} onChange={handleSheetChange} className="p-2 border rounded-md">
            {sheetOptions.map((sheet) => (<option key={sheet.name} value={sheet.name}>
                {sheet.name}
              </option>))}
          </select>
        </div>
      ...
      <div>
        <RankingTable data={ranking} highlightIndex={highlightedIndex} ascending={currentSheet?.ascending}/>
      </div>
    </div>);
}
